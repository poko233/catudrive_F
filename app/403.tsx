...
function buildSvgColors(themeName: string, primary: string): SvgThemeColors {
  const fixed = {
    skin: "#F2C094",
    shirtDark: "#1A1A2E",
    shirtLight: "#E8E0D0",
  ...
  if (themeName === "premium") {
    bgBlob = "rgba(212,175,55,0.18)";
    errorText = "#FDF6E3";
  } else if (themeName === "dark") {
    bgBlob = "rgba(239,68,68,0.18)";
    errorText = "#F8FAFC";
  } else {
    bgBlob = "rgba(211,47,47,0.12)";
    errorText = "#111827";
  }

  return { primary, bgBlob, errorText, ...fixed };
}

/* ─── SVG del policía (texto original restaurado) ─── */
const PoliceSvg = ({
  colors,
  maxWidth = 500,
}: {
  ...
  return (
    <Svg width={svgWidth} height={svgHeight} viewBox="0 0 500 500">
      {/* Blob de fondo */}
      <Path
      ...
      {/* ── Textos originales ERROR / 403 / FORBIDDEN ── */}
      <G>
        <Path
        ...
        <Path
          ...
          fill={colors.errorText}
        />
        <Path d="M371.55,254.73v21.8h-2.43v-21.8Z" fill={colors.errorText} />
        <Path
      ...
      <G>
        ...
        />
        <Polygon
          points="395.49 440.56 344.39 440.56 364.42 331.6 375.46 331.6"
          fill={colors.primary}
          stroke={colors.shirtDark}
          strokeLinecap="round"
          strokeLinejoin="round"
        ...
        />
        <Polygon
          points="356.98 372.1 382.91 372.1 380.42 358.6 359.46 358.6"
          fill={colors.barrierAlt}
          stroke={colors.shirtDark}
          strokeLinecap="round"
          strokeLinejoin="round"
...
/* ─── Partícula flotante con repulsión ─── */
const FloatingShape = ({
  size,
  top,
  left,
...
export default function ForbiddenScreen() {
  ...
  // 50 partículas en cuadrícula 10x5
  ...
  const svgColors: SvgThemeColors = useMemo(
    () => buildSvgColors(theme.name, theme.colors.primary),
    [theme],
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <PoliceSvg colors={svgColors} maxWidth={svgMaxWidth} />
        </Animated.View>
        ...
        >
          <View
            style={{
              ...
              height: isMobile ? 70 : 80, // logo más alto
