...
export default function EncomiendasScreen(){
 ...
 const acciones=(e:Encomienda)=><View style={styles.actions}>
  <Visibility action="Ver" selector=".encomiendas-ver"><IconButton icon={Eye} size="sm" variant="secondary" accessibilityLabel="Ver encomienda" onPress={()=>setDetalle(e)}/></Visibility>
  {e.qr_disponible?<Visibility action="Ver" selector=".encomiendas-qr"><IconButton icon={QrCode} size="sm" variant="secondary" accessibilityLabel="QR e impresión" onPress={()=>void abrirTicket(e)}/></Visibility>:null}
  {e.estado==="EN_ORIGEN"?<Visibility action="Editar" selector=".encomiendas-editar"><IconButton icon={Pencil} size="sm" variant="secondary" accessibilityLabel="Editar encomienda" onPress={()=>setEditing(e)}/></Visibility>:null}
  {(e.estado==="EN_ORIGEN"||e.estado==="EN_TRANSITO")?<Visibility action="Editar" selector=".encomiendas-editar"><IconButton icon={ArrowRight} size="sm" variant="secondary" accessibilityLabel={e.estado==="EN_ORIGEN"?"Marcar en tránsito":"Marcar en destino"} loading={processingId===e.id} onPress={()=>void avanzar(e)}/></Visibility>:null}
  {e.estado==="EN_DESTINO"&&e.estado_pago==="Pendiente"?<Visibility action="Editar" selector=".encomiendas-editar"><IconButton icon={Banknote} size="sm" variant="secondary" accessibilityLabel="Cobrar encomienda" onPress={()=>setCobro(e)}/></Visibility>:null}
  {e.estado==="EN_DESTINO"?<Visibility action="Editar" selector=".encomiendas-entregar"><IconButton icon={PackageCheck} size="sm" variant="secondary" accessibilityLabel="Entregar encomienda" loading={processingId===e.id} onPress={()=>void entregarItem(e)}/></Visibility>:null}
  {!esChofer&&e.estado==="EN_ORIGEN"?<Visibility action="Editar" selector=".encomiendas-anular"><IconButton icon={XCircle} size="sm" variant="destructive" accessibilityLabel="Anular encomienda" loading={processingId===e.id} onPress={()=>void anular(e)}/></Visibility>:null}
 </View>;
 ...
 const listado=<View style={styles.listWrap}>
  ...
  {mobile?<View style={styles.cards}>{loading&&encomiendas.length===0?<ActivityIndicator color={c.primary}/>:encomiendas.map(e=><Card key={e.id} style={styles.mobileCard}>
    <View style={styles.mobileHead}><View><ThemedText style={styles.bold}>{e.guia??"—"}</ThemedText><ThemedText style={{color:c.textSecondary}}>{fecha(e.created_at)}</ThemedText></View><Badge label={estadoLabel(e.estado)} variant={estadoVariant(e.estado)}/></View>
    <ThemedText>{nombre(e.remitente)} → {nombre(e.destinatario)}</ThemedText><ThemedText style={{color:c.textSecondary}}>{e.origen??"—"} → {e.destino??"—"}</ThemedText>
    <ThemedText>{cantidad(e)} bulto(s) · Bs {Number(e.total).toFixed(2)} · {e.estado_pago}</ThemedText><View style={styles.mobileActions}>{acciones(e)}</View>
  </Card>)}</View>:
