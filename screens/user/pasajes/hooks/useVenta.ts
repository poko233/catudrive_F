...
import { usePasajesStore } from "../store/pasajesStore";

export function useVenta() {
  const [ventaActual, setVentaActual] =
  ...
  const confirmar = useCallback(
    async (
      ...
      try {
        const pasajerosStore =
          usePasajesStore
            .getState()
            .pasajeros;

        const pasajerosConId =
          pasajeros.map(
            (pasajero, index) => ({
              ...pasajero,
              id_pasajero:
                pasajerosStore[index]
                  ?.id_pasajero ??
                null,
            }),
          );
